#' ---
#' title: "EU-level posterior probability LOO analysis - N-1 cross validation"
...
#------------------------------------------------------------------------------------------------
df2b <- df2 %>% filter(trachoma_cat != "Unclassified")
eus <- unique(df2b$eu) 
prob_df_elim_LOO <- data.frame()

for(e in 1:(length(eus))){
  print(e)
  print(eus[e])
  # exclude each EU from entire dataset iteratively:
  df3 <- subset(df2b, eu != eus[e])
  
  # probability of SCR in each category => likelihood:
  #1) group data into categories:
  scr_elim <- df3 %>% filter(trachoma_cat == "Action not needed")
  scr_endemic <- df3 %>% filter(trachoma_cat == "Action needed")
  ...
  ## below computes posterior probability for post-elimination
  A <- foreach(i=prior_elim, k=prior_endemic) %:%
    foreach(c=scr_elim$scr, .combine = 'rbind', .multicombine = TRUE) %dopar% {
      ...
      data.frame(e, eus[e], i, c, prob_elim_c)
    }
...
colnames(prob_df_elim_LOO) <- c("LOO_set", "EU_left", "prior_set", "c", "prob")

write.csv(prob_df_elim_LOO, "output/probs_scr_elimination-eu-N1-2categories.csv", row.names = FALSE)
saveRDS(prob_df_elim_LOO, "output/probs_scr_elimination-eu-N1-2categories.rds")
