#' ---
#' title: "Plot Bayesian SCR estimates - Fig 2"
#' author: ekamau
...
h <- ggplot(data = df2 %>% filter(trachoma_cat != "Unclassified")) +
  ...
  #ggforce::facet_col(~fct_relevel(trachoma_cat, "Action needed","Action not needed"), scales = "free_y", space = "free") +
  labs(x = "", y = "Survey") +
  theme_minimal() + 
...
ggsave(filename = here("output/", "Fig2.png"), A, width = 6, height = 8, dpi = 300)
